...
{
  /// <summary>
  /// Represents 『大辞泉』 (Daijisen).
  /// J-J.
  /// Japanese-only example sentences.
  /// </summary>
  [Serializable]
  public class DicEpwingDaijisen : DicEpwing
  {
    /// <summary>
    /// Constructor.
    /// </summary>
    ...
    private void parseBody(List<string> lines, Entry entry, bool includeExamples, FineTune fineTune)
    {
      ...
      {
        string lineWithNewlines = Regex.Replace(line, "([①-⑳㉑-㊿❶-❾㋐-㋾㊀-㊉◆Ⅰ-Ⅻ])", "\n$1");
        string[] subLines = lineWithNewlines.Split(new string[] { "\n" }, StringSplitOptions.RemoveEmptyEntries);
        ...
        foreach (string subLine in subLines)
        {
          ...
          if ((defLine == "[画像]") || (defLine == "[動画]") || (defLine == "♪"))
          {
            continue;
          }
    ...
    private string processExampleBlanks(string example, string expression, bool fillInBlanks)
    {
      ...
      if (fillInBlanks)
      {
        ...
        if (unformattedExpression.Contains('|'))
        {
          ...
          {
            // Don't care
            Logger.Instance.error("daijisen.processExampleBlanks: " + e1);
          }
        }
    ...
    private bool parseFirstLine(string line, Entry entry)
    {
      ...
      Match match = Regex.Match(line, @"(?:^(?<Reading>.*?)【(?<Expression>.*?)】)|(?:^(?<Reading>.*))");
      ...
      if (match.Success)
      {
        ...
        if ((entry.Reading.Length > 0)
          && UtilsLang.containsAlpha(entry.Reading[0]))
        {
          success = false;
        }
        else
